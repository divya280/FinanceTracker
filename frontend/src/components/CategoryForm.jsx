...
const CategoryForm = ({ isOpen, onClose, onSubmit, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({ name: initialData.name, type: initialData.type, color: initialData.color || '#6366f1' });
    } else {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-16 h-10 rounded-md border border-input bg-background"
            />
          </div>
